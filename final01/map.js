    ...
    let fareRates = {
        traditional: { base_rate: 13.00, per_kmrate: 1.80 },
...
async function loadFareRates() {
    ...
    function getDistanceToNearestStop(lat, lng) {
...
function redrawLockedRoute() {
    ...
    if (currentLength === _lastRedrawLength) return;
    ...
    // Include current stop so gray connects with blue
    ...
    if (remainingPath.length >= 2) {
        drawJeepneySegments(remainingPath);
    }
}
    ...
    async function processUserLocation(lat, lng, force = false) {
        ...
        if (distanceToMainRoute > 1000) {
            showToast("Your current location is not near the main route.");
            stopTracking();
            ...
            setStatusMessage("Out of tracking range.", "warning");
            return; 
        }
...
if (distToICCT <= ICCT_WALK_RADIUS) {
   ...
   showToast(`Just walk! You're ${Math.round(distToICCT)}m from ICCT (~${walkMinutes} min). No jeepney needed.`, 6000);
    ...
    return;
            ...
            if (!startStop) {
                showToast("No nearby terminal or stop found in service areas.");
                return;
            }
            ...
            // Evaluate the drop-off options using the 1km limit
            stops.forEach(stop => {
                ...
                // CRITICAL CHANGE: Ignore stops further than 1km from ICCT
            ...
            if (!bestEndStop) {
                ...
                renderRouteDetails();
                showToast("No jeepney drop-off point found within 1km of ICCT.");
                return;
            }
    ...
    drawJeepneySegments(path);

globalActivePathData = { path, startStop, bestEndStop };
            await loadFareRates();
            renderRouteDetails();
            setStatusMessage("Route ready. Lock it to start navigating.", "success");

            // Draw final walking leg to campus gate
            const lastStop = stopById[path[path.length - 1]];
            if (lastStop) {
                const finalWalkDist = map.distance([lastStop.lat, lastStop.lng], ICCT_POSITION);
                // double-check distance to safely paint the final path line
                if (finalWalkDist <= ICCT_WALK_RADIUS) {
    ...
    // JEEPNEY TYPE TOGGLE
    window.setJeepneyType = async function (type) {
    ...
    function renderRouteDetails() {
        ...
        const { startStop, bestEndStop } = globalActivePathData;
    ...
    const flushSegment = () => {
            ...
            const regFare  = calculateJeepneyFare(currentSegmentDistance, selectedJeepneyType, false);
            const discFare = calculateJeepneyFare(currentSegmentDistance, selectedJeepneyType, true);
    ...
    input.addEventListener("input", debounce(async () => {
        ...
        const url = `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(q)}&filter=rect:${BOUNDS.minLon},${BOUNDS.minLat},${BOUNDS.maxLon},${BOUNDS.maxLat}&apiKey=${API_KEY}`;
        ...
        try {
            ...
            const nearRouteFeatures = (data.features || []).filter(place => {
                ...
                return stops.some(stop =>
                    map.distance([lat, lon], [stop.lat, stop.lng]) <= 750
                );
            });
